...
plotLCOE <- function(LCOEfile, gdx, y = c(2015, 2020, 2030, 2040, 2050, 2060),
                     reg = "all_regi", fileName = "LCOE_plots.pdf") {
  ...
  plot.cost <- c(
    "Flex Tax", "Second Fuel Cost", "CO2 Tax Cost", "Fuel Cost",
  ...
  plot_theme <- theme_bw() + theme(
    axis.text.x = element_text(angle = 90, size = 40, vjust = 0.3),
  ...
  cost.colors <- c(
    "Investment Cost" = "azure4", "OMF Cost" = "darkcyan", "OMV Cost" = "cyan",
  ...
  qBalSe.m <- readGDX(gdx, "q_balSe", field = "m", restore_zeros = FALSE)
  qBalSeel.m <- readGDX(gdx, "q32_balSe", field = "m", restore_zeros = FALSE)
  budget.m <- readGDX(gdx, "qm_budget", field = "m")[, getYears(qBalSe.m), ]
  vm_prodSe <- readGDX(gdx, "vm_prodSe", field = "l", restore_zeros = FALSE)

  vm_prodSe_output <- dimSums(vm_prodSe, dim = c(3.1, 3.3), na.rm = TRUE) # sum SE over all technologies per output type
  ...
  df.LCOE.dC.join <- df.LCOE.in %>%
    ...
    filter(
      cost == "Investment Cost" | variable == "LCOE",
  ...
  make_plot <- function(df.LCOE.grouped) {
    ...
    df.LCOE.plot <- df.LCOE.grouped %>%
      ...
      mutate(TooHigh = ifelse(variable == "LCOE", ifelse((`Total LCOE` - ifelse(`CO2 Tax Cost` < 0, `CO2 Tax Cost`, 0)
        - ifelse(`Second Fuel Cost` < 0, `Second Fuel Cost`, 0)) > ylimit.up, TRUE, FALSE), FALSE)) %>%
    ...
    sec.axis.limit <- max(df.dC.max$value, na.rm = TRUE) + 1 # second axis max
    ...
    p.LCOE <- ggplot() +
      geom_col(
        data = df.LCOE.plot.out %>% filter(variable == "LCOE"),
        aes(tech, value, fill = cost), alpha = 0.7
      ) +
      geom_point(
        data = df.LCOE.plot.out %>% filter(variable == "vm_deltaCap"),
        aes(tech, value, color = variable),
        alpha = 0.8, size = 9
      ) +
      geom_hline(
        data = df.LCOE.plot.out %>% filter(variable == "Price"),
